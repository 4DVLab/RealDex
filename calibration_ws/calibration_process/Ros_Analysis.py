...
def BagDataSave(BagPath,output_path, folder_index: str, topics):
    ...
    for (depthinfo_topic, depthinfo_msg, depthinfo_t),\
            ...
            print(depthinfo_t, depthimage_t, rgbinfo_t, rgbimage_t)
            scale = 2.0  # 缩小四倍，这样跟原来的depth就会比较接近
            ...
            SaveImage(OutputPath, "ColorUndistort",
                    "{:06d}-color.png".format(image_num), undistorted_rgb_image)
            SaveImage(OutputPath, "colorDistort",
                    "{:06d}-DistortedRGBImage.png".format(image_num), rgb_cv_image)
...
def RosBagSaveTime(BagPath, output_path, folder_index: str, topics):
    ...
    with open(OutputPath, 'a+') as file:
        for (depthinfo_topic, depthinfo_msg, depthinfo_t),\
                ...
                (rgbimage_topic, rgbimage_msg, rgbimage_t) \
            in tqdm(zip(depth_image_info, depth_image_raw, rgb_image_info, rgb_image_raw)):
            ros_time = [depthinfo_t, depthimage_t, rgbinfo_t, rgbimage_t]
            file.write(f"{depthinfo_t} {depthimage_t} {rgbinfo_t} {rgbimage_t}\n")
    BagData.close()
...
if __name__ == "__main__":
    ProgramInit()
    output_path = "/media/tony/新加卷/Bags2InitialPose/Data/1/"
    post_name = ['/depth/camera_info', '/depth/image_raw',
                 '/rgb/camera_info', '/rgb/image_raw']
    predix = '/cam'
    cam_num = 4
    BagPath = "/media/tony/新加卷/Bags2InitialPose/yangtao_1_20230905.bag"
    output_path = "/media/tony/新加卷/Bags2InitialPose/Data/2/"
    for index in np.arange(cam_num):
        topics = [predix + str(index) + item for item in post_name]
