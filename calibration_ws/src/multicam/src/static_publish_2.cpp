...
{
    ros::init(argc, argv, "static_publish_2");
    ros::NodeHandle nh;
    ...
    while(nh.ok())
    {
        ...
        try
        {
            for(int i=0;i<2;i++)
            {
                listener.lookupTransform("/cam"+std::to_string(i)+"_camera_base", "/cam"+std::to_string(i)+"_depth_camera_link", ros::Time(0), cam_transform[i]);
