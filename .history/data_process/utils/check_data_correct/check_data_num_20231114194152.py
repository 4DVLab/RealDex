...
class img_data(ABC):
    def __init__(self,camera_folder_path) -> None:
        data_num = 0
        time_stamp_file = False
        img_index_continuous = False
        self.img_nums = 0
        self.camera_folder_ = camera_folder_path
    ...
    def check_img_continue(self):
        img_file_names = list(os.listdir(self.folder_path))
        img_file_names = list(map(int,img_file_names))
        img_file_names.sort()
        for index in np.arange(0,len(img_file_names)):
            if img_file_names[index] != index:
                self.img_index_continuous = False
                break
