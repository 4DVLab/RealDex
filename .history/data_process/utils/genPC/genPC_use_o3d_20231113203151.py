...
def gen_aligned_pc(bag_data_path:str,cam_nums = [0,1,2,3],img_nums = [num for num in range(0,2000,1)]):#set a range 
    ...
    for cam_index in cam_nums:
        ...
        for rgb_img_index in img_nums:
            ...
            pcd = gen_pcd_with_depth_and_rgb(rgb_camera_param,depth_camera_param,rgb_img_folder,str(rgb_img_index),depth_img_folder,str(depth_img_index))
            rgb_img_path = rgb_img_folder / Path(str(rgb_img_index) + ".png")
            depth_img_path = depth_img_folder / Path(str(depth_img_index) + ".png")
