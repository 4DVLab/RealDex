...
def get_target_folders(root_folder:Path,folder_postfixs: List[Path] = []) ->None:
...
def gen_aligned_pc(bag_data_path:str,cam_nums = [0,1,2,3],img_nums = [num for num in range(0,2000,1)]):#set a range 
    ...
    for cam_index in cam_nums:
        ...
        depth_msg_folder,rgb_msg_folder = Path(cam_folder) / Path(image_prefix_folder[0])
         = Path(cam_folder) / Path(image_prefix_folder[1])
