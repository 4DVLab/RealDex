...
def gen_pcd_with_depth_and_rgb(rgb_param,
                               depth_param,
                               rgb_img_folder,
                               rgb_img_name,
                               depth_img_folder,
...
def gen_aligned_pc(bag_data_path:str,cam_nums = [0,1,2,3],img_nums = [num for num in range(0,2000,1)]):#set a range 
    ...
    for cam_index in cam_nums:
        ...
        #以depth为主来生成点云
        for rgbimg_index in img_nums:
            if img_index >= rgb_stamps_length:#enhance the robust of this program
                return
            
            depth_index = find_time_closet(rgb_stamp[img_index], depth_stamp)



            gen_pcd_with_depth_and_rgb(rgb_camera_param,depth_camera_param,rgb_img_folder,str(rgb_index),depth_img_folder,img_index)
