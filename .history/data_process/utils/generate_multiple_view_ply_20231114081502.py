    ...
    def merge_pcd_and_filter(self):
        ...
        for time_stamp in self.all_cams_time_stamp_list[0]:
            ...
            for cam_index in np.arange(4):
                ...
                pcd = self.gen_one_cam_pcd(cam_index,index)
                four_cams_pcd.append(pcd)
            merge_pcd = self.merge_four_cams_pcd(four_cams_pcd)
            merge_pcd = self.filter_pcd(merge_pcd)
            o3d.io.write_point_cloud(str(self.merge_pcd_save_folder / Path(str(time_stamp)+".ply")), merge_pcd, write_ascii=False, compressed=False, print_progress=True)
            print(f"save the {time_stamp} pcd")
