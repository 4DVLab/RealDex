...
def get_index_tracking_result(bag_path):
    bag_name = bag_path.split("/")[-1]
    tracking_result_file_path = bag_path / f"{bag_name}_cam_index"

def get_new_gt_transform_with_icp(bag_path,new_begin_tracking_index):
...
def backward_nsteps(bag_path, n_steps_backward, cam_index):
    ...
    new_gt_tranform = get_new_gt_transform_with_icp(bag_path,new_begin_tracking_index,cam_index)
