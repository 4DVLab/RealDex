...
class gen_merge_pcd_ply:
    ...
    def merge_pcd_and_filter(self,constrain_bound):
        ...
        os.makedirs(self.merge_pcd_save_folder / f"TEMP",exist_ok=tRU)
        for index in np.arange(constrain_bound[0],constrain_bound[1]):
            ...
            for cam_index in np.arange(3,4):
                ...
                o3d.io.write_point_cloud(str(
                    self.merge_pcd_save_folder / f"cam{index}_index{index}.ply"), pcd, write_ascii=False, compressed=False, print_progress=True)
            #     merge_pcd += pcd
    ...
    def filter_pcd(self, pcd):  # for every pcd, we have to filter the point cloud
        ...
        pcd = pcd.crop(o3d.geometry.AxisAlignedBoundingBox(
            np.array([0.868298, -0.431623, 0.811648], np.float64), np.array([1.6193, 0.519053, 1], np.float64)))
        # pcd = pcd.crop(o3d.geometry.AxisAlignedBoundingBox(
