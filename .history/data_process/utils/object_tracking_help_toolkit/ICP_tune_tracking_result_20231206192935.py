...
def icp_tune_tracking_result(folder_path):
    ...
    for index in tqdm(np.arange(0,223)):
        ...
        seven_num = transform_matrix_to_seven_num(tracking_result_list[index])
        # seven_num = transform_matrix_to_seven_num(final_transform_matrix)

            # transform_list = np.array(transform_list)
        with open(str(folder_path / f"tracking_result/tracking_and_icp.txt"),"a+") as data_saver:
            np.savetxt(np.array(seven_num).reshape((1,-1)), data_saver)
