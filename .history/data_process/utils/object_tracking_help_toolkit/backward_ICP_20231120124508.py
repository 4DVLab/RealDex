...
def get_index_tracking_result(bag_path, cam_index):
    bag_name = bag_path.split("/")[-1]
    tracking_result_file_path = bag_path / f"{bag_name}_cam_index_{cam_index}_tracking_resul"


def get_new_gt_transform_with_icp(bag_path, new_begin_tracking_index, cam_index):
    cam0rgb_to_world_transform = load_cam0rgb_to_world_transform(bag_path)


def backward_nsteps(bag_path, n_steps_backward, cam_index):

    new_begin_tracking_index = load_backtrack_index(bag_path,n_steps_backward)

    new_gt_tranform = get_new_gt_transform_with_icp(
        bag_path, new_begin_tracking_index, cam_index)


if __name__ == "__main__":
    args = get_args()  
    backward_nsteps(args.bag_path, args.backtrack_steps,args.cam_index)
