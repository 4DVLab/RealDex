...
class pcd_iter():
    ...
    def __next__(self):
        if self.pcd_index < self.pcd_num:
            pcd_path = Path(self.folder_path) / Path(f"{self.file_name_prefix}{self.pcd_index}{self.file_name_postfix}.pcd")
...
def icp_tune_tracking_result(folder_path):
    pcd_iter = load_pcd_iter(folder_path / f"merged_pcd_filter",file_name_prefix="merge_pcd_",file_name_postfix="_simplified")if __name__ == "__main__":
