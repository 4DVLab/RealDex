...
def ICP_between_two_pcd(source_mesh_model,target_pcd,point_to_point=True):
    if point_to_point:
        source_pcd = source_mesh_model.sample_points_uniformly(number_of_points=10000)
        ...
        source_point_cloud.points = source_mesh_model.vertices
        source_mesh_model.compute_vertex_normals()
        source_mesh_model.normals = source_mesh_model.vertex_normals
        icp_result = o3d.pipelines.registration.registration_icp(
        source_pcd, target_pcd, max_correspondence_distance=0.004,#最大的crospondence距离，不能设置太小了，不然找不到对应点
...
def get_new_gt_transform_with_icp(bag_path, new_begin_tracking_index, cam_index, mesh_model_name):
    ...
    icp_transform = ICP_between_two_pcd(mesh_model_transform_to_world_frame_with_tracking_result,env_pcd)
    
    mesh_model_icp_in_cam0_frame_transform = np.linalg.inv(cam0rgb_to_world_transform)@icp_transform
...
def write_transform_to_gt_file(bag_path,transform,gt_index):
    bag_path = Path(bag_path)
    file_path = bag_path / f"poses/gt_{gt_index}.txt"
...
def backward_nsteps(bag_path, n_steps_backward, cam_index, mesh_model_name):
    ...
    write_transform_to_gt_file(bag_path, new_gt_tranform,new_begin_tracking_index)
