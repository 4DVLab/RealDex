...
def transform_obj2result_pose(bag_folder_path,model_name,cam2world_transform = None):

    mesh_path = Path(bag_folder_path) / Path("models") /  Path(model_name + ".obj")

    mesh = o3d.io.read_triangle_mesh(str(mesh_path))
    # mesh = simplify_mesh(mesh)
    pcd = mesh.sample_points_poisson_disk(9000)#don't use mesh simplify, but the sample poi
    print("simple mesh done")
    pose_path = Path(bag_folder_path) / Path("pose_result") /Path("result.txt")
    ...
    #         future.result()
    for index in np.arange(tranform_matrixs.shape[0]):
...
if __name__ == "__main__":
    ...
    transform_obj2result_pose(bag_folder_path,model_name,cam0_rgb_camera_link2world)
