...
class gen_merge_pcd_ply:
    def __init__(self,bag_folder,four_cam_intrisics_extrisics_save_folder) -> List[str,Dict[str,int]]:
        ...
        self.four_cams_to_world_frame = [self.cam0_transform_to_world @ self.extrics[f"{cam_index}"] for cam_index in np.arange(1,4,1)]
        self.four_cams_to_world_frame  = self.four_cams_to_world_frame.insert(0,self.cam0_transform_to_world)
    
    def get_all_cams_to_world_frame(self,cams_inter_transform,cam0_to_world_transform):
        four_cams_to_world_frame = [self.cam0_to_world_transform @
                                    self.cams_inter_transform[f"{cam_index}"] for cam_index in np.arange(1, 4, 1)]
        four_cams_to_world_frame = four_cams_to_world_frame.insert(0, self.cam0_to_world_transform)
