...
# so, later, we gen the point cloud from the four camera, and then we use the four camera to gen the point cloud
class gen_multi_view_ply:
    def __init__(self,root_path,four_cam_intrisics_extrisics_save_folder) -> List[Dict[str,int]]:
        """
        use four_cam_transform_folder to transform the point cloud
        """
        intrisics,extrics = get_four_cam_intrisics_extrisics(four_cam_intrisics_extrisics_save_folder)
    def get_four_cam_intrisics_extrisics(self,)->dict:
        return 
