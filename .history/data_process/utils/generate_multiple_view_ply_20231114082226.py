...
class gen_merge_pcd_ply:
    ...
    def gen_pcd_with_depth_and_rgb_paths(
        ...
        depth_img_path = self.bag_folder / Path(f"cam{cam_index}/rgb/image_raw/{str(depth_img_index)}.png")

        undistorted_rgb = self.undistort_image(rgb_img_path, rgb_param["K"], rgb_param["D"])
        undistorted_rgb = cv2.cvtColor(undistorted_rgb, cv2.COLOR_BGR2RGB)

        undistorted_depth = self.undistort_image(depth_img_path, depth_param["K"], depth_param["D"])
