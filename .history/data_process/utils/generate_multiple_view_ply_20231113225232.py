...
class trisics(TypedDict):




# so, later, we gen the point cloud from the four camera seperatly
class gen_multi_view_ply:
    def __init__(self,root_path,four_cam_intrisics_extrisics_save_folder) -> List[str,Dict[str,int]]:
        """
    ...
    def load_one_camera_intrisics(self,cam_intrisics_folder:Path,cam_num:Path)->Dict[str,int]:
        ...
        with open(cam_intrisics_path) as json_reader:
    ...
    def load_internal_camera_extrisics(self,cam_extrisics_folder:Path,cam_num:Path):
        cam_extrisics_path = cam_extrisics_folder / f"cali0{cam_num}_rgb.txt"
        extrics = {"rotation":None,"translation":None}
        with open(cam_extrisics_path) as json_reader:
            camera_data = json.load(json_reader)
            extrics["rotation"] = np.array(camera_data["R"]).reshape(3,3)
            extrics["translation"] = np.array(camera_data["T"]).flatten()
        return extrics
    def get_four_cam_intrisics_extrisics(self,four_cam_intrisics_extrisics_save_folder)->dict:
        intr
        for cam_index in np.arange(1,4,1):
            with open(four_cam_intrisics_extrisics_save_folder / Path(f"cam0{cam_index}.json")) as json_reader:
                data = json.load(json_reader)
                intrisics = data["intrisics"]
                extrics = data["extrics"]
                intrisics_list.append(intrisics)
                extrics_list.append(extrics)
