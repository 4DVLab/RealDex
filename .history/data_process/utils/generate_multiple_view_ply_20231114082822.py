...
class gen_merge_pcd_ply:
    ...
    def gen_pcd_with_depth_and_rgb_paths(
        ...
        rgb_img_path = self.bag_folder / Path(f"cam{cam_index}/rgb/image_raw/{str(rgb_img_index)}.png")
        depth_img_path = self.bag_folder / Path(f"cam{cam_index}/rgb/image_raw/{str(rgb_img_index)}.png")
    ...
    def gen_cams_time_stamp(self):
        ...
        for cam_index in np.arange(4):
            cam_time_stamp = np.loadtxt(self.bag_folder / Path(f"cam{cam_index}/rgb/image_raw/info.txt"))
            all_cams_time_stamp_list.append(cam_time_stamp)
        all_cams_align_to_cam0_rgb_time_stamps = [all_cams_time_stamp_list[0],[],[],[]]
        for cam_index in np.arange(1,4):
            for cam_time_stamp in all_cams_time_stamp_list[0]:
                index = common.find_time_closet(cam_time_stamp,all_cams_time_stamp_list[cam_index])
                all_cams_align_to_cam0_rgb_time_stamps[cam_index].append(all_cams_time_stamp_list[cam_index][index])
        return all_cams_align_to_cam0_rgb_time_stamps


    def merge_pcd_and_filter(self):
        self.all_cams_time_stamp_list = self.gen_cams_time_stamp()
        for time_stamp in self.all_cams_time_stamp_list[0]:
            for cam_index in np.arange(4):
                index = common.find_time_closet(time_stamp,self.all_cams_time_stamp_list[cam_index])
                pcd = self.gen_pcd_with_depth_and_rgb_paths(index,cam_index)
                pcd.transform(self.four_cams_to_world_frame[cam_index])
                pcd = self.filter_pcd(pcd)
                o3d.io.write_point_cloud(str(self.merge_pcd_save_folder / Path(str(time_stamp)+".ply")), pcd, write_ascii=False, compressed=False, print_progress=True)
