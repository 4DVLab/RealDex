...
def load_transform(folder_path):
    ...
    with open(global_transform_path,"r") as global_transform_reader:
        global_transform = json.load(global_transform_reader)
...
if __name__ == "__main__":
    ...
    transform_matrix = np.array(
              [[-0.978030 ,0.023425, -0.207145 ,1.454866],
                [0.148005 ,0.777795 ,-0.610843 ,0.598974],
                [0.146807 ,-0.628081 ,-0.764174 ,1.309810],
                [0.000000 ,0.000000, 0.000000 ,1.000000]])
    print("init transform_matrix")
    print(transform_matrix)
    folder_path = "/media/tony/新加卷/test_data/test/test_1"
    global_transforms = load_transform(folder_path)
    cam2world_transform = np.array(global_transforms["cam0_rgb_camera_link"])
    print("world2cam0_transform")
    world2cam0 = np.linalg.inv(cam2world_transform)
    print(world2cam0)
    transform_matrix = world2cam0 @ transform_matrix
    print("cam0frame_init_transform_matrix")
    print(transform_matrix)
    print(Rotation.from_matrix(transform_matrix[:3,:3]).as_quat())
