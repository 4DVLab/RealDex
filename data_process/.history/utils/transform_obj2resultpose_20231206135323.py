...
def load_transform(folder_path, bag_name):
    global_transform = Path(
        folder_path) / Path("global_name_position") / Path("0.txt")
    
    with open(global_transform,"r") as global_transform_reader:
        global_transform = np.loadtxt(global_transform_reader,dtype=np.float32)
