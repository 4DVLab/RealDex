...
def load_global_transform(folder_path, bag_name):
    cam2world_transform_path = Path(
        folder_path) / Path(global_name_position) / Path("cam2world_transform.txt")
    cam2world_transform = np.loadtxt(cam2world_transform_path,dtype=np.float32)
    return cam2world_transform
