...
from project_hand_arm_mesh_2_2d.project_hand_arm_mesh_2_2d import load_camera_param
...
def output_merge_mesh(meshes,bag_folder,index,simplify = True):
    ...
    for key,value in meshes.items():
        ...
        if simplify:
            value = value.simplify_quadric_decimation(int((1-0.9) * len(value.triangles)))
        merged_mesh += value
...
def change_o3d_camera_param(o3d_camera_param_path,mine_camera_param):
    ...
    extrisic = [item for list_item in mine_camera_param["extrinsic"] for item in list_item]
    ...
    o3d_camera_param["intrinsic"]["intrinsic_matrix"] = chang_trinsic(mine_camera_param["intrinsic"])
...
def viz_arm_hand_mesh_without_bag(config_folder,bag_folder,rosbag_predix, base_frame = "world",output_jud = False,my_cam = True):#这个path在bag文件夹之上，equal to 
    ...
    camera_param_path = config_folder / Path("project_arm_hand_to_2d/camera_params.json")
...
if __name__ == "__main__":
    ...
    viz_arm_hand_mesh_without_bag(configuration_path,bag_folder,ros_prefix_path)
