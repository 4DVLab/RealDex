...
def process_pcd(index, pcd_folder, tracking_result_list, model_name, transform_list):
    ...
    transform_list[f"{index}"] = seven_num

    dict_to_array_and_save(pcd_folder.parent, transform_list)
    return seven_num


def dict_to_array_and_save(folder_path, transform_dict):

    # 找到最小和最大的键值
    max_key = int(max(transform_dict.keys()))

    # 创建一个包含所有可能键值的矩阵，并将缺失的键值对应的行填充为0
    matrix_size = max_key + 1
    matrix = np.zeros((matrix_size, 7))
    ...
    with open(folder_path / f"tracking_result/tracking_and_icp.txt", "w+") as data_saver:
                    np.savetxt(data_saver, matrix.reshape((-1,7)))
...
def multi_thread_icp_tune_tracking_result(folder_path, model_name, constrain_bound, cam_parameter_path=None, max_workers=None):
    ...
    with ProcessPoolExecutor(max_workers=8, mp_context=ctx) as executor:
        ...
        future_to_index = {executor.submit(
            process_pcd, index, pcd_folder, tracking_result_list[index], model_name,transform_list): index for index in index_range}
