...
class ShadowHandBuilder():
    ...
    def _load_mesh_from_urdf(self, urdf_path, prefix):
        ...
        for i, link in enumerate(trimesh_dict.keys()):
            ret_dict[link] = {
                ...
                # 'points': sampled_pts[i].unsqueeze(0).to(self.device),
                # 'points_normal': pts_normal[i].unsqueeze(0).to(self.device)
            }
    ...
    def get_hand_model(self, world_rotation, world_translation, qpos):
        ...
        for link_name in self.mesh_dict:
            ...
            # print(link_name, current_status[link_name])
            
            verts = current_status[link_name].transform_points(mesh['vertices'])
            
            
            # faces_normal = torch.tensor(mesh['points_normal'])
            face_normals = current_status[link_name].transform_normals(mesh['face_normals'])
            
            # pts = torch.tensor(mesh['points'])
            # pts = current_status[link_name].transform_points(mesh['points'])
            # points_normal = torch.tensor(mesh['points_normal'])
            # point_normals = current_status[link_name].transform_normals(mesh['point_normals'])
            ...
            # pts = torch.einsum('bik,bjk->bij', pts, world_rotation)
            # pts = pts + world_translation
            
            face_normals = torch.einsum('bik,bjk->bij', face_normals, world_rotation)
            # point_normals = torch.einsum('bik,bjk->bij', point_normals, world_rotation)            
            
            ret_dict['verts'].append(verts)
            ret_dict['faces'].append(f)
            # ret_dict['points'].append(pts)
            ret_dict['face_normals'].append(face_normals)
            # ret_dict['point_normals'].append(point_normals)
