...
class PCDGenerator:
    ...
    def merge_pcd_and_filter(self,constrain_bound):
        ...
        for cam_index in np.arange(4):
            cam_data_save_folder_path = os.path.join(self.merge_pcd_save_folder, f"cam{cam_index}")
            os.makedirs(cam_data_save_folder_path,exist_ok=True)
        os.makedirs(os.path.join(self.merge_pcd_save_folder, f"TEMP"),exist_ok=True)
        for index in np.arange(constrain_bound[0],constrain_bound[1]):
            ...
            for cam_index in range(4):
                ...
                o3d.io.write_point_cloud(os.path.join(self.merge_pcd_save_folder, f"cam{cam_index}", f"{index}.ply"), 
                                        pcd, write_ascii=False, compressed=False, print_progress=True)

                
                # o3d.io.write_point_cloud(os.path.join(self.merge_pcd_save_folder, f"TEMP/cam{cam_index}_index{index}.ply"), 
                #                         pcd, write_ascii=False, compressed=False, print_progress=True)
            #     merge_pcd += pcd
    ...
    def init_merge_pcd_timestamp(self):
        shutil.copy2(
            os.path.join(self.bag_folder, "cam0/rgb/image_raw/info.txt"), self.merge_pcd_save_folder)
        os.makedirs(self.merge_pcd_save_folder, exist_ok=True)
...
def gen_severa_annotate_pcd(bag_folder, four_cam_intrisics_extrisics_save_folder, constrain_bound):
    ...
    gen_merge_pcd = PCDGenerator(
        bag_folder, four_cam_intrisics_extrisics_save_folder)

    gen_merge_pcd.merge_pcd_and_filter(constrain_bound)


def gen_pcd_for_annotate(root_path, four_cam_intrisics_extrisics_save_folder, constrain_bound):
...
if __name__ == "__main__":
    four_cam_intrisics_extrisics_save_folder = "../../calibration_ws/calibration_process/data"
    
    root_path = "/Users/yumeng/Working/data/CollectedDataset/sprayer_1_20231209/"

    
    
    constrain_bound = [0,10]
