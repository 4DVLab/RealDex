...
def viz_project_object_to_2d(object_folder,my_cam=True):
    ...
    vis.create_window()
    camera_param_path = os.path.join(object_folder, "camera_param.json")
    if my_cam:
    ...
    image_save_folder = os.path.join(object_folder, f"capture_image")

    os.makedirs(image_save_folder, exist_ok=True)
    for object_index in np.arange(223):
        object_path = os.path.join(object_folder, f"object_pose_in_every_frame_with_icp/{object_index}.ply")
        ...
        plt.imsave(image_save_folder /Path(f"{object_index}.png"), np.asarray(image), dpi=1)
