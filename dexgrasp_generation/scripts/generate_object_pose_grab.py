...
def generate_object_pose(_):
    ...
    if not args.overwrite and os.path.exists(os.path.join(args.data_root_path, object_name, 'poses.npy')):
        return
    ...
    if os.path.exists(os.path.join(args.data_root_path, object_name, 'coacd', 'coacd.urdf')) and not os.path.exists(os.path.join(args.data_root_path, object_name, 'coacd', 'coacd_convex_piece_63.obj')):
        loader = scene.create_urdf_loader()
        loader.fix_root_link = False
        object_actor = loader.load(os.path.join(args.data_root_path, object_name, 'coacd', 'coacd.urdf'))
        object_actor.set_name(name='object')
    else:
        builder = scene.create_actor_builder()
        builder.add_collision_from_file(os.path.join(args.data_root_path, object_name + ".ply"))
    ...
    np.save(os.path.join(args.data_root_path, object_name, 'poses.npy'), pose_matrices)
    
    # remove convex hull
    if os.path.exists(os.path.join(args.data_root_path, object_name+".ply.convex.stl")):
        os.remove(os.path.join(args.data_root_path, object_name+".ply.convex.stl"))
...
if __name__ == '__main__':
    ...
    with Pool(args.n_cpu) as p:
        ...
        for object_file in object_file_list:
            ...
            object_path = os.path.join(args.data_root_path, object_name)
            if not os.path.exists(object_path):
                os.makedirs(object_path)
            param_list.append((args, object_name))
