...
def train(args, epoch, model, train_loader, device, optimizer, log_root, rh_mano, rh_faces):
    ...
    for batch_idx, (obj_pc, hand_xyz, hand_param, obj_cmap) in enumerate(train_loader):
        ...
        # cmap loss
        cmap_loss = CMap_loss(obj_pc.permute(0,2,1)[:,:,:3], recon_xyz, obj_cmap, rh_faces)
        loss = a * cvae_loss + b * param_loss + c * cmap_loss
        loss.backward()
        optimizer.step()
        ...
        if batch_idx % args.print_every == 0 or batch_idx == len(train_loader) - 1:
            print("Train Epoch {:02d}/{:02d}, Batch {:04d}/{:d}, weight {}, Total Loss {:9.5f}, Mesh loss {:9.5f}, KLD loss {:9.5f}, Param loss {:9.5f}, CMap loss {:9.5f}".format(
                    epoch, args.epochs, batch_idx, len(train_loader) - 1, str(args.loss_weight), loss.item(),
                    recon_loss_num, KLD_loss_num, param_loss.item(), cmap_loss.item()))

    time_elapsed = time.time() - since
    out_str = "Epoch: {:02d}/{:02d}, train, time {:.0f}m, weight {}, Mean Toal Loss {:9.5f}, Mesh loss {:9.5f}, KLD loss {:9.5f}, Param loss {:9.5f}, CMap loss {:9.5f}".format(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--loss_weight", type=list, default=[1.0, 0.1, 1.0])
