...
class affordanceNet(nn.Module):
    ...
    def forward(self, obj_pc, hand_param):
        '''
        ...
        if self.training:
            recon, means, log_var, z = self.cvae(hand_param, obj_glb_feature) # recon: [B, 778*3]
...
if __name__ == '__main__':
    model = affordanceNet(obj_inchannel=4,
                          cvae_encoder_sizes=[61, 512, 256],
                          cvae_decoder_sizes=[1024, 256, 61])
    obj_xyz = torch.randn(3, 4, 3000)
    hand_param = torch.randn(3, 61)
