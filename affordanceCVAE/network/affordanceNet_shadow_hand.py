...
class affordanceNet(nn.Module):
    def __init__(self, cvae_encoder_sizes, cvae_decoder_sizes, obj_inchannel=4, cvae_latent_size=64, cvae_condition_size=1024):
    ...
    def forward(self, obj_pc, qpos):
        '''
        :param obj_pc: [B, 3+n, N1]
        :param qpos: [B, 22]
        :return: reconstructed qpos
        '''
        B = qpos.shape[0]
        ...
        if self.training:
            recon, means, log_var, z = self.cvae(qpos, obj_glb_feature) # recon: [B, 22]
            recon = recon.contiguous().view(B, qpos_dim)
