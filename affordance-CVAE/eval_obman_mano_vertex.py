...
def eval(args, model, eval_loader, device, rh_mano):
    ...
    model.eval()
    with torch.no_grad():
        for batch_idx, (obj_pc, hand_xyz_gt, hand_param, obj_cmap, img) in enumerate(eval_loader):
            obj_pc, hand_xyz_gt, hand_param = obj_pc.to(device), hand_xyz_gt.to(device), hand_param.to(device)
            recon_param = model(obj_pc, hand_xyz_gt)
